...
for level in stripe raid10 raidz1 raidz2 raidz3 mirror; do

	for pgconf in default no-fpw no-fpw-tuned; do
			...
			if [ "$level" == "raid10" ]; then
				sudo zpool create -o ashift=12 -R /mnt zfs_raid mirror /dev/sda /dev/sdb mirror /dev/sdc /dev/sdd mirror /dev/sde /dev/sdf > $TESTDIR/zpool.create.log 2>&1
			else
				sudo zpool create -o ashift=12 -R /mnt zfs_raid $l /dev/sda /dev/sdb /dev/sdc /dev/sdd /dev/sde /dev/sdf > $TESTDIR/zpool.create.log 2>&1
			fi
